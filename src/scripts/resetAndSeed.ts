...
async function run() {
  ...
  const branch = await Branch.findOne({ email: TARGET_EMAIL });
  if (!branch) {
  ...
  const [fees, members, services, staff] = await Promise.all([
    Fee.deleteMany({ branch: b }),
    Member.deleteMany({ branch: b }),
    Service.deleteMany({ branch: b }),
    Staff.deleteMany({ branch: b }),
  ]);
  ...
  // ── Seed Services ───────────────────────────────────────────────────────────
  console.log('\nSeeding services...');
  const [yoga, cardio, strength, pilates, swimming, martialArts, dance, nutrition] =
  ...
  for (const def of memberDefs) {
    ...
    const inserted = await Member.findById(result.insertedId);
    createdMembers.push({ id: inserted!._id as mongoose.Types.ObjectId, name: def.name, svcs: def.svcs });
  ...
  const dueTargets = createdMembers.filter((m) =>
