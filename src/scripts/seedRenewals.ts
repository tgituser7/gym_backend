...
async function upsertService(data: Record<string, unknown>) {
  const exists = await Service.findOne({ branch: data.branch, name: data.name });
...
async function upsertMember(data: Record<string, unknown>) {
  const exists = await Member.findOne({ branch: data.branch, name: data.name });
  if (exists) {
    // keep renewal dates relative to today on each run
    await Member.updateOne({ _id: exists._id }, { membershipEndDate: data.membershipEndDate });
    return exists;
  }
  ...
  if (data.email) doc.email = (data.email as string).toLowerCase().trim();
  if (data.phone) doc.phone = String(data.phone).trim();
  const result = await Member.collection.insertOne(doc);
  console.log(`  + member:  ${data.name}`);
  // Return a Mongoose document by fetching what was just inserted
  return Member.findById(result.insertedId) as ReturnType<typeof Member.findById>;
}

async function upsertFee(data: Record<string, unknown>) {
  const exists = await Fee.findOne({ branch: data.branch, member: data.member, dueDate: data.dueDate });
  if (exists) return exists;
  const doc = await Fee.create(data);
...
async function seed() {
  ...
  if (!branch) { console.error('No active branch found.'); process.exit(1); }
  ...
  const createdMembers: { id: mongoose.Types.ObjectId; name: string; svcs: mongoose.Types.ObjectId[] }[] = [];
  for (const def of memberDefs) {
    ...
    createdMembers.push({ id: (doc as { _id: mongoose.Types.ObjectId })._id, name: def.name, svcs: def.svcs });
  }
  ...
  const pendingTargets = createdMembers.filter((m) =>
    ['Rohit Desai', 'Meena Iyer', 'Suresh Kumar', 'Vikram Singh', 'Pooja Agarwal',
  ...
  const overdueTargets = createdMembers.filter((m) =>
    ['Rahul Verma', 'Fatima Khan', 'Divya Nair'].includes(m.name)
  );
